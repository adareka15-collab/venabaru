...
import { getFreelancerPortalData, freelancerPortalAction } from '../lib/api';
...
const FreelancerPortal: React.FC<FreelancerPortalProps> = ({ accessId, teamMembers, projects, teamProjectPayments, teamPaymentRecords, rewardLedgerEntries, showNotification, onUpdateRevision, sops, profile }) => {
    ...
    const [portalData, setPortalData] = useState<{
        ...
        teamPaymentRecords: TeamPaymentRecord[];
        sops: SOP[];
    }>({ freelancer: null, projects: [], teamProjectPayments: [], teamPaymentRecords: [], sops: [] });
    ...
    useEffect(() => {
        const loadPortalData = async () => {
            try {
                setLoading(true);
                const result = await getFreelancerPortalData(accessId);
                setPortalData(result.data);
            } catch (err) {
                console.error('Error loading portal data:', err);
                setError('Gagal memuat data portal');
                // Fallback to props data
                const freelancer = teamMembers.find(m => m.portalAccessId === accessId);
                const assignedProjects = projects.filter(p => p.team.some(t => t.memberId === freelancer?.id));
                const freelancerPayments = teamProjectPayments.filter(p => p.teamMemberId === freelancer?.id);
                const freelancerRecords = teamPaymentRecords.filter(r => r.teamMemberId === freelancer?.id);
                setPortalData({ 
                    freelancer: freelancer || null, 
                    projects: assignedProjects, 
                    teamProjectPayments: freelancerPayments,
                    teamPaymentRecords: freelancerRecords,
                    sops: sops
                });
            } finally {
                setLoading(false);
        ...
        loadPortalData();
    }, [accessId, teamMembers, projects, teamProjectPayments, teamPaymentRecords, sops]);

    const freelancer = portalData.freelancer;
    const assignedProjects = portalData.projects;
        ...
        <div className="min-h-screen bg-brand-bg text-brand-text-primary p-4 sm:p-6 lg:p-8">
            <div className="max-w-5xl mx-auto">
                ...
                <Modal isOpen={!!selectedProject} onClose={() => setSelectedProject(null)} title={`Detail Proyek: ${selectedProject?.projectName}`} size="3xl">
                    {selectedProject && <ProjectDetailModal project={selectedProject} freelancer={freelancer} onUpdateRevision={async (projectId, revisionId, updatedData) => {
                        try {
                            await freelancerPortalAction(accessId, 'update_revision', { projectId, revisionId, updatedData });
                            onUpdateRevision(projectId, revisionId, updatedData);
                        } catch (error) {
                            console.error('Error updating revision:', error);
                            showNotification('Gagal memperbarui revisi. Silakan coba lagi.');
                        }
                    }} showNotification={showNotification} onClose={() => setSelectedProject(null)} />}
                </Modal>
...
const DashboardTab: React.FC<{freelancer: TeamMember, projects: Project[], teamProjectPayments: TeamProjectPayment[]}> = ({ freelancer, projects, teamProjectPayments }) => {
    const stats = useMemo(() => {
        const unpaidFee = teamProjectPayments.filter(p => p.status === 'Unpaid').reduce((sum, p) => sum + p.fee, 0);
        const paidFee = teamProjectPayments.filter(p => p.status === 'Paid').reduce((sum, p) => sum + p.fee, 0);
