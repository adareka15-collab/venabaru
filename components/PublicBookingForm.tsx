...
import { submitBookingForm, dbOperations } from '../lib/api';
...
const PublicBookingForm: React.FC<PublicBookingFormProps> = ({ 
    ...
    // Load packages and add-ons from Supabase on component mount
    React.useEffect(() => {
        const loadData = async () => {
            try {
                const { packages: dbPackages, addOns: dbAddOns } = await dbOperations.getPackagesAndAddOns();
                setAvailablePackages(dbPackages);
                setAvailableAddOns(dbAddOns);
            } catch (error) {
                console.error('Error loading packages and add-ons:', error);
    ...
    }, [formData.packageId, formData.selectedAddOnIds, formData.promoCode, availablePackages, availableAddOns, promoCodes]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const bookingData = {
            ...
            dpPaymentRef: formData.dpPaymentRef,
        ...
        try {
            ...
            setIsSubmitted(true);
            showNotification('Pemesanan berhasil dikirim!');
        } catch (error) {
